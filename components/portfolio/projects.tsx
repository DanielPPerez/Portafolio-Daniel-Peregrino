    ...
    <section id="proyectos" className="py-24">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <Reveal>
          ...
          <p className="mt-3 max-w-2xl text-foreground/60">{t.projects.subtitle}</p>
        </Reveal>

        <div className="mt-12 grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {t.projects.items.map((project, i) => (
            <Reveal key={project.title} delay={i * 0.1}>
              <article className="group flex h-full flex-col gap-4 rounded-xl border border-border bg-white/[0.02] p-4 transition-all hover:border-neon-purple/40 hover:glow-border-purple">
                <BrowserFrame
                  url={project.url}
                  title={project.title}
                ...
                <div className="flex flex-1 flex-col">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {project.tags.map((tag) => (
                      <span
                        key={tag}
                        className="rounded-md border border-border bg-white/5 px-2 py-0.5 font-mono text-xs text-neon-blue"
                      >
                        {tag}
                      </span>
                    ))}
